  ...
  try {
    const { rows } = await query(`
      SELECT
        ...
        COUNT(*)::int AS orders,
        COALESCE(SUM(user_price), 0)::numeric(12,2) AS revenue
      FROM orders
  ...
  try {
    const { rows } = await query(`
      SELECT
        ...
        COUNT(*) FILTER (WHERE status IN ('cancelled','expired','refunded'))::int AS failed_orders,
        COUNT(*) FILTER (WHERE status = 'received')::int AS completed_orders,
  ...
  try {
    const { rows } = await query(`
      SELECT
        ...
        COALESCE(SUM(user_price) FILTER (WHERE status = 'received'), 0)::numeric(12,2) AS total_revenue
      FROM orders
      WHERE created_at >= NOW() - INTERVAL '30 days'
      GROUP BY service
