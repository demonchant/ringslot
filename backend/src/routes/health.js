...
router.get('/health/providers', (_req, res) => {
  try {
    ...
    const connections = Object.values(stats);
    const available = connections.filter((s) => s.isAvailable).length;
    const allHealthy = connections.length > 0 && available === connections.length;
    res.status(allHealthy ? 200 : 503).json({
      status: allHealthy ? 'ok' : 'degraded',
      connections: { available, configured: connections.length },
    });
  } catch (err) {
