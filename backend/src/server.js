...
import { initializeAccessControl } from './services/accessControl.js';
...
app.use(cors({
  origin: (origin, cb) => {
    // Server-to-server clients usually have no Origin. Browser origins must
    // match the production allowlist above.
...
async function boot() {
  try {
    ...
    if (process.env.ENABLE_PROVIDER_MONITORS !== 'false') {
      ...
      logger.info('Provider and price monitors disabled for this environment');
    }
